...
DOC_MIME = {"application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/msword"}
PPT_MIME = {"application/vnd.ms-powerpoint", "application/vnd.openxmlformats-officedocument.presentationml.presentation"}
EXCEL_MIME = {"application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}
...
async def split_document(file: UploadFile = File(...), page_ranges: str = Form(...)):
    ...
    if len(out_files) == 1:
        return FileResponse(out_files[0], filename=os.path.basename(out_files[0]), media_type='application/pdf')
    
    zip_path = os.path.join(TEMP_DIR, f"split_{uuid.uuid4()}.zip")
    with zipfile.ZipFile(zip_path, 'w') as zipf:
        for f in out_files:
            zipf.write(f, os.path.basename(f))
    return FileResponse(zip_path, filename="split_files.zip", media_type='application/zip')
...
async def pdf_to_word(file: UploadFile = File(...)):
    ...
    return FileResponse(out_path, filename="converted.docx", media_type=list(DOC_MIME)[0])
...
async def pdf_to_ppt(file: UploadFile = File(...)):
    ...
    return FileResponse(out_path, filename="converted.pptx", media_type=list(PPT_MIME)[1])
...
async def pdf_to_excel(file: UploadFile = File(...)):
    ...
    return FileResponse(out_path, filename="converted.xlsx", media_type=list(EXCEL_MIME)[1])
...
async def sign_pdf(
    ...
    PDFManipulator().add_signature(doc_path, sig_path, out_path, page_num, x, y, width, height)
...
async def watermark_pdf(
    ...
    PDFManipulator().add_watermark(path, text, out_path, opacity, rotation, font_size, color)
    return FileResponse(out_path, filename="watermarked.pdf", media_type='application/pdf')

@router.post("/rotate")
async def rotate_pdf(file: UploadFile = File(...), rotation: int = Form(...), page_nums: Optional[str] = Form(None)):
...
async def organize_pdf(file: UploadFile = File(...), page_order: str = Form(...)):
    ...
    return FileResponse(out_path, filename="organized.pdf", media_type='application/pdf')
...
async def ocr_pdf(file: UploadFile = File(...), lang: str = Form("eng")):
    ...
    return FileResponse(out_path, filename="ocr_result.pdf", media_type='application/pdf')
...
async def compare_pdfs(file1: UploadFile = File(...), file2: UploadFile = File(...)):
    ...
    return FileResponse(out_path, filename="comparison_diff.pdf", media_type='application/pdf')
