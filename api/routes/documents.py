...
from api.models import APIResponse, DocumentSession, CanvasData, TextAnnotation, ImageAnnotation, MetadataUpdate
from api.deps import (
    sessions, create_session, get_session, delete_session, 
    persist_session_document, TEMP_DIR, MAX_UPLOAD_MB
)
from pdfsmarteditor.utils.canvas_helpers import (
    render_page_image, parse_fabric_objects, validate_canvas_object,
    scale_coordinates, convert_to_pymupdf_annotation, decode_canvas_overlay
)
...
async def commit_canvas(doc_id: str, page_num: int, canvas_data: CanvasData):
    ...
    for obj in objects:
        if not validate_canvas_object(obj): continue
